...
class RobotiqGripper:
    ...
    def activate(self, auto_calibrate: bool = True, calibration_cache_path: Optional[str] = None):
        """Resets the activation flag in the gripper, and sets it back to one, clearing previous fault flags.
        :param auto_calibrate: Whether to calibrate the minimum and maximum positions based on actual motion.
        :param calibration_cache_path: Path to cache calibration results. If valid cache exists (<24h), skip physical calibration.
        The following code is executed in the corresponding script function
        ...
        if auto_calibrate:
            ...
            if calibration_cache_path:
                cache_loaded = self._load_calibration_cache(calibration_cache_path)

            if cache_loaded:
                # Move to open position using cached values
                self.move_and_wait_for_pos(self.get_open_position(), 64, 1)
            else:
                self.auto_calibrate(min_position=min_position, max_position=max_position)
                if calibration_cache_path:
                    self._save_calibration_cache(calibration_cache_path)

    def _load_calibration_cache(self, path: str) -> bool:
        """Load cached calibration values if file exists and is less than 24 hours old."""
        try:
            with open(path, "r") as f:
                data = json.load(f)
            ts = data.get("timestamp", 0)
            if time.time() - ts > 86400:
                print(f"Calibration cache expired ({path})")
                return False
            self._min_position = int(data["min_position"])
            self._max_position = int(data["max_position"])
            print(f"Loaded calibration cache: [{self._min_position}, {self._max_position}] from {path}")
            return True
        except (FileNotFoundError, json.JSONDecodeError, KeyError, OSError) as e:
            print(f"Cannot load calibration cache: {e}")
            return False

    def _save_calibration_cache(self, path: str) -> None:
        """Save current calibration values to cache file."""
        try:
            data = {
            ...
            print(f"Cannot save calibration cache: {e}")
    ...
    def auto_calibrate(self, log: bool = True, min_position: Optional[int] = None, max_position: Optional[int] = None) -> None:
        """Attempts to calibrate the open and closed positions, by slowly closing and opening the gripper.
        :param log: Whether to print the results to log.
        """

        # Apply optional position limits before physical calibration
        if min_position is not None:
            self._min_position = min_position
        if max_position is not None:
            self._max_position = max_position

        print(f"Auto-calibrating gripper to [{self._min_position}, {self._max_position}]")

        # Physical open/close calibration (always executed)
        ...
        print(f"Gripper closed position: {position}")
        self._max_position = max(position, self._min_position)

        # try to open as far as possible, and record the number
        (position, status) = self.move_and_wait_for_pos(self.get_open_position(), 64, 1)
        if RobotiqGripper.ObjectStatus(status) != RobotiqGripper.ObjectStatus.AT_DEST:
            raise RuntimeError(f"Calibration failed because of an object: {str(status)}")
        self._min_position = min(position, self._max_position)
