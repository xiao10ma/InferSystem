...
DELTA_EEF_STATE_DIM = 20
CAMERA_NAME_MAP = {
    "third_view": "observation.image.third_view",
    "left_wrist_view": "observation.image.left_wrist_view",
    "right_wrist_view": "observation.image.right_wrist_view",
}
...
CAMERA_NAMES = ["third_view", "left_wrist_view", "right_wrist_view"]
...
def inference_fn_non_blocking_fast(args, config, policy, ros_operator):
    ...
    while not rospy.is_shutdown():
        try:
            ...
            payload = {
                "observation/image": imgs[0].transpose(2, 0, 1),
                "observation/left_wrist_image": imgs[1].transpose(2, 0, 1),
                "observation/right_wrist_image": imgs[2].transpose(2, 0, 1),
                "state": proprio,
            ...
            if args.ctrl_type != "delta_eef":
                actions = scale_gripper(actions, scale=3.0)
...
def scale_gripper(actions: np.ndarray, scale: float = 10.0):
    ...
    for idx in gripper_indices:
        if actions.shape[1] > idx:
            actions[:, idx] *= scale
    return actions


def _normalize_vec(vec, fallback):
...
def rot6d_to_rotmat(rot6d):
    """Convert Zhou-style 6D rotation representation to a 3x3 rotation matrix."""
    rot6d = np.asarray(rot6d, dtype=float).reshape(6)
    a1 = rot6d[:3]
    a2 = rot6d[3:6]
    b1 = _normalize_vec(a1, [1.0, 0.0, 0.0])
    a2_orth = a2 - np.dot(b1, a2) * b1
    b2 = _normalize_vec(a2_orth, [0.0, 1.0, 0.0])
    b3 = np.cross(b1, b2)
...
def rotmat_to_rot6d(rotmat):
    ...
    return np.concatenate([rotmat[:, 0], rotmat[:, 1]], axis=0)


def euler_xyz_to_rotmat(euler_rad):
    rx, ry, rz = np.asarray(euler_rad, dtype=float).reshape(3)
...
def rotmat_to_euler_xyz(rotmat):
    rotmat = np.asarray(rotmat, dtype=float).reshape(3, 3)
    sy = -rotmat[2, 0]
    ...
    cy = math.cos(ry)
    if abs(cy) > 1e-6:
        rx = math.atan2(rotmat[2, 1], rotmat[2, 2])
        rz = math.atan2(rotmat[1, 0], rotmat[0, 0])
    else:
        rx = math.atan2(-rotmat[1, 2], rotmat[1, 1])
        rz = 0.0
...
def clamp_rotmat_step(current_rot, target_rot, max_angle_rad):
    if max_angle_rad is None or max_angle_rad <= 0:
        return target_rot
    current_rot = np.asarray(current_rot, dtype=float).reshape(3, 3)
    target_rot = np.asarray(target_rot, dtype=float).reshape(3, 3)
    delta_rot = current_rot.T @ target_rot
    cos_theta = float(np.clip((np.trace(delta_rot) - 1.0) / 2.0, -1.0, 1.0))
    theta = math.acos(cos_theta)
    if theta <= max_angle_rad or theta < 1e-9:
        return target_rot
    axis = np.array([
    ...
    if axis_norm < 1e-8:
        return target_rot
    axis = axis / axis_norm
    return current_rot @ rotvec_to_rotmat(axis * max_angle_rad)


def clamp_vec_step(current, target, max_step):
    ...
    if norm <= max_step or norm < 1e-12:
        return target
    return current + delta / norm * max_step
...
def get_config(args):
    state_dim = DELTA_EEF_STATE_DIM if getattr(args, "ctrl_type", "joint") == "delta_eef" else 14
    config = {
...
def update_observation_window(args, config, ros_operator):
    ...
    if getattr(args, "ctrl_type", "joint") == "delta_eef":
        qpos = ros_operator.get_eef_state()
    else:
        qpos = np.concatenate(
...
def inference_fn(args, config, policy):
    ...
    while True and not rospy.is_shutdown():
        ...
        payload = {
            "observation/image": image_arrs[0].transpose(2, 0, 1),
            "observation/left_wrist_image": image_arrs[1].transpose(2, 0, 1),
            "observation/right_wrist_image": image_arrs[2].transpose(2, 0, 1),
            "state": proprio,
            "prompt": lang_embeddings,
        }
            ...
            while t < max_publish_step and not rospy.is_shutdown() and not shutdown_event.is_set():             
                ...
                if args.use_temporal_smoothing:
                    ...
                    if act is not None:
                        ...
                        elif args.ctrl_type == "delta_eef":
                            left_action, right_action = ros_operator.delta_eef_publish(act)
                            published_actions_history.append(np.concatenate([left_action, right_action], axis=0).astype(float))

                        else:
                            print("Make sure ctrl_type is joint or delta_eef")
                    else:
                        print("act is None")
                        time.sleep(0.001)
                        continue
...
class RosOperator:
    ...
    def _read_single_arm(self, arm):
        ...
        try:
            ...
            scale = 1000000.0 if getattr(self.args, "ctrl_type", "joint") == "delta_eef" else 10000000.0
            gripper_m = float(gripper.gripper_state.grippers_angle) / scale
        except Exception:
            pass
    ...
    def _read_single_endpose(self, arm):
        msg = arm.GetArmEndPoseMsgs().end_pose
        x = float(getattr(msg, "X_axis")) / 1000000.0
        y = float(getattr(msg, "Y_axis")) / 1000000.0
        z = float(getattr(msg, "Z_axis")) / 1000000.0
        rx = float(getattr(msg, "RX_axis")) / 1000.0 * DEG_TO_RAD
        ry = float(getattr(msg, "RY_axis")) / 1000.0 * DEG_TO_RAD
        rz = float(getattr(msg, "RZ_axis")) / 1000.0 * DEG_TO_RAD
        return np.array([x, y, z, rx, ry, rz], dtype=float)

    def get_endpose_snapshots(self):
        return self._read_single_endpose(self.left_arm), self._read_single_endpose(self.right_arm)

    def _single_eef_state(self, arm):
        eef6 = self._read_single_endpose(arm)
        gripper_m = self._read_single_arm(arm)[6]
        rot6d = rotmat_to_rot6d(euler_xyz_to_rotmat(eef6[3:6]))
        return np.concatenate([eef6[:3], rot6d, [gripper_m]], axis=0)

    def get_eef_state(self):
        return np.concatenate([self._single_eef_state(self.left_arm), self._single_eef_state(self.right_arm)], axis=0)

    def _publish_single_endpose(self, arm, eef6, gripper_m):
        eef6 = np.asarray(eef6, dtype=float).reshape(6)
        xyz_sdk = [int(round(float(v) * 1000000.0)) for v in eef6[:3]]
        rpy_sdk = [int(round(float(v) * RAD_TO_DEG * 1000.0)) for v in eef6[3:6]]
        gripper_sdk = int(round(float(gripper_m) * 1000000.0))
        gripper_sdk = max(0, min(80000, gripper_sdk))
        ...
        if callable(motion_ctrl_2):
            motion_ctrl_2(0x01, 0x00, int(self.args.arm_speed_percent), 0x00)
        arm.EndPoseCtrl(*(xyz_sdk + rpy_sdk))
        arm.GripperCtrl(gripper_sdk, 1000, 0x01, 0)

    def _apply_delta_eef(self, current_eef6, delta10):
        current_eef6 = np.asarray(current_eef6, dtype=float).reshape(6)
        delta10 = np.asarray(delta10, dtype=float).reshape(10)
        target_xyz = current_eef6[:3] + delta10[:3]
        current_rot = euler_xyz_to_rotmat(current_eef6[3:6])
        delta_rot = rot6d_to_rotmat(delta10[3:9])
        target_rot = current_rot @ delta_rot
        target_rpy = rotmat_to_euler_xyz(target_rot)
        gripper = float(delta10[9])
        return np.concatenate([target_xyz, target_rpy, [gripper]], axis=0)

    def _absolute_eef10_to_target(self, eef10):
        eef10 = np.asarray(eef10, dtype=float).reshape(10)
        target_xyz = eef10[:3]
        target_rpy = rotmat_to_euler_xyz(rot6d_to_rotmat(eef10[3:9]))
        gripper = float(eef10[9])
        return np.concatenate([target_xyz, target_rpy, [gripper]], axis=0)

    def _limit_absolute_eef_target(self, current_eef6, current_gripper, target):
        target = np.asarray(target, dtype=float).reshape(7).copy()
        current_eef6 = np.asarray(current_eef6, dtype=float).reshape(6)
        max_xyz_step = float(getattr(self.args, "max_eef_xyz_step", 0.0))
        max_rot_step = float(getattr(self.args, "max_eef_rot_step_deg", 0.0)) * DEG_TO_RAD
        max_gripper_step = float(getattr(self.args, "max_eef_gripper_step", 0.0))

        target[:3] = clamp_vec_step(current_eef6[:3], target[:3], max_xyz_step)

        current_rot = euler_xyz_to_rotmat(current_eef6[3:6])
        target_rot = euler_xyz_to_rotmat(target[3:6])
        target[3:6] = rotmat_to_euler_xyz(clamp_rotmat_step(current_rot, target_rot, max_rot_step))

        target[6] = float(clamp_vec_step(np.array([current_gripper]), np.array([target[6]]), max_gripper_step)[0])
        return target

    def absolute_eef_publish(self, action):
        action = np.asarray(action, dtype=float).reshape(-1)
        left_current, right_current = self.get_endpose_snapshots()
        left_gripper = self._read_single_arm(self.left_arm)[6]
        right_gripper = self._read_single_arm(self.right_arm)[6]

        if action.size == 20:
            left_target = self._absolute_eef10_to_target(action[:10])
            right_target = self._absolute_eef10_to_target(action[10:20])
        elif action.size == 10:
            left_target = np.concatenate([left_current, [left_gripper]], axis=0)
            right_target = self._absolute_eef10_to_target(action)
        else:
            raise ValueError(f"absolute eef action must have 10 or 20 values, got {action.size}")

        left_target = self._limit_absolute_eef_target(left_current, left_gripper, left_target)
        right_target = self._limit_absolute_eef_target(right_current, right_gripper, right_target)

        self._eef_grip_log_n = getattr(self, "_eef_grip_log_n", 0) + 1
        if self._eef_grip_log_n % 20 == 0:
            left_sdk = max(0, min(80000, int(round(float(left_target[6]) * 1000000.0))))
            right_sdk = max(0, min(80000, int(round(float(right_target[6]) * 1000000.0))))
            print(f"[delta_eef gripper] L={left_target[6]:+.5f} sdk={left_sdk:+d} | "
                  f"R={right_target[6]:+.5f} sdk={right_sdk:+d}", flush=True)

        self._publish_single_endpose(self.left_arm, left_target[:6], left_target[6])
        self._publish_single_endpose(self.right_arm, right_target[:6], right_target[6])
        return left_target, right_target

    def delta_eef_publish(self, action):
        # The current aloha_wipe_board server trains in delta-eef space but applies
        # AbsoluteActions on outputs, so the websocket result is an absolute eef target.
        return self.absolute_eef_publish(action)
    ...
    def puppet_arm_publish(self, left, right):
        ...
        self.left_arm.GripperCtrl(left_gripper_sdk, 1000, 0x01, 0)
        self.right_arm.GripperCtrl(right_gripper_sdk, 1000, 0x01, 0)

    def endpose_publish(self, left, right):
        self._publish_single_endpose(self.left_arm, left[:6], left[6])
        self._publish_single_endpose(self.right_arm, right[:6], right[6])
    ...
    def get_frame(self):
        ...
        if not required.issubset(images.keys()):
            ...
            return False
        left_snap, right_snap = self.get_joint_snapshots()
        img_front = images[CAMERA_NAME_MAP['third_view']]
        img_left = images[CAMERA_NAME_MAP['left_wrist_view']]
        img_right = images[CAMERA_NAME_MAP['right_wrist_view']]
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        default="/home/ubuntu/InferSystem/Config/aloha_dj.yaml",
        required=False,
    )
    parser.add_argument(
        "--can_interface_left",
    ...
    parser.add_argument(
        "--max_eef_xyz_step",
        type=float,
        help="Maximum xyz motion per publish step for delta_eef absolute targets, meters; <=0 disables",
        default=0.005,
        required=False,
    )
    parser.add_argument(
        "--max_eef_rot_step_deg",
        type=float,
        help="Maximum rotation motion per publish step for delta_eef absolute targets, degrees; <=0 disables",
        default=5.0,
        required=False,
    )
    parser.add_argument(
        "--max_eef_gripper_step",
        type=float,
        help="Maximum gripper motion per publish step for delta_eef absolute targets; <=0 disables",
        default=0.004,
        required=False,
    )

    args = parser.parse_args()
    if args.use_delta_eef_smoothing:
        # The dedicated delta-eef smoother is not implemented; use the same
        # chunk-level temporal smoother that the delta-eef path currently supports.
        args.use_temporal_smoothing = True
    return args
